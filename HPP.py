...
class HPP(object):
	def __init__(self, path):
		...
		self.Block_txyz = np.zeros(self.NT*self.NX0*self.NY0*self.NZ0 *
		                           4).reshape(self.NT, self.NX0, self.NY0, self.NZ0, 4)
		self.Block_txyz[:, :, :, :, 0] = self.data_t[:, 0].reshape(
		    self.NT, self.NX0, self.NY0, self.NZ0)  # Ed
		self.Block_txyz[:, :, :, :, 1] = self.data_t[:, 1].reshape(
		    self.NT, self.NX0, self.NY0, self.NZ0)  # Vx
		self.Block_txyz[:, :, :, :, 2] = self.data_t[:, 2].reshape(
		    self.NT, self.NX0, self.NY0, self.NZ0)  # Vy
		self.Block_txyz[:, :, :, :, 3] = self.data_t[:, 3].reshape(
		    self.NT, self.NX0, self.NY0, self.NZ0)  # Vz
...
# change 3D to 2D directly
	def change3Dto2D(self):
		OutPut2D = self.Block_txyz[:, :, :, self.NZ0//2,
		    0:3].reshape(self.NT*self.NX0*self.NY0, 3)
...
def main(data_root_path, Nx,Ny, Nz, Deltx, Delty,Deltz,Event_number, Dim_switch,Grids_switch,sed,st,sfrac,sv,outputpath):
	...
	for i in range(0,Event_number):
		...
		if Dim_Switch == 2 and NX == doit.NX0 and NY ==doit.NY0 and DeltX == doit.DX0 and DeltY == doit.DY0:
			print('Start to change file directly!')
			doit.todo = doit.change3Dto2D()
			doit.save()
			continue
		if Dim_Switch ==3 and NX == doit.NX0 and NY ==doit.NY0 and NZ == doit.NZ0 and DeltX == doit.DX0 and DeltY == doit.DY0 and DeltZ == doit.DZ0:
